...
#     def cargar_pdf(self):
#         self.pdf_path = filedialog.askopenfilename(filetypes=[("Archivos PDF", "*.pdf")], initialdir=os.getcwd())
#         if self.pdf_path:
#             self.label_pdf.config(text=f"PDF: {os.path.basename(self.pdf_path)}")
#             self.verificar_archivos_cargados()

#     def cargar_excel(self):
#         self.excel_path = filedialog.askopenfilename(filetypes=[("Archivos Excel", "*.xlsx *.xls")], initialdir=os.getcwd())
#         if self.excel_path:
#             try:
#                 df_excel = pd.read_excel(self.excel_path, dtype=str)  # Leer todo como string
...
#                 self.label_excel.config(text=f"Excel: {os.path.basename(self.excel_path)}")
#                 self.verificar_archivos_cargados()
...
#         output_default = f"{os.path.splitext(os.path.basename(self.pdf_path))[0]} {fecha_actual}.xlsx"
#         self.output_file = filedialog.asksaveasfilename(defaultextension=".xlsx", initialfile=output_default, filetypes=[("Archivos Excel", "*.xlsx")])

#         if self.output_file:
#             data = extraer_datos_pdf_parallel(self.pdf_path, num_workers=self.num_workers, progress_callback=self.actualizar_progreso)
#             self.label_progreso.config(text="Creando archivo Excel, por favor espere...")
#             guardar_en_excel(data, self.output_file, self.pdf_path, self.excel_path, num_workers=self.num_workers)
